...
public class MainMenuManager : MonoBehaviour, ILobbyCallbacks, IInRoomCallbacks, IOnEventCallback, IConnectionCallbacks, IMatchmakingCallbacks {
    ...
    public const int NICKNAME_MIN = 2, NICKNAME_MAX = 32;
    ...
    private static readonly string roomNameChars = "ABCDFGHJKLMNPRQSTVWXYZ0123456789";
    ...
    public void OnJoinedLobby() {
        PlayerPrefs.GetInt("PlayerRank", 1000);
        Hashtable prop = new() {
    ...
    public void OnPlayerPropertiesUpdate(Player player, Hashtable playerProperties) {
        ...
        if (PhotonNetwork.IsMasterClient && Utils.GetCharacterIndex(player) > 4 && !debug) {
    ...
    public void OnMasterClientSwitched(Player newMaster) {
        ...
        if (newMaster.IsLocal) {
            ...
            LocalChatMessage("You are the room's host! You can click on player names to control your room, or use chat commands. Do /help for more help.", Color.green);
        }
        UpdateSettingEnableStates();
    }
    public void OnJoinedRoom() {
        Debug.Log($"[PHOTON] Joined Room ({PhotonNetwork.CurrentRoom.Name})");
        LocalChatMessage(PhotonNetwork.LocalPlayer.GetUniqueNickname() + " joined the lobby (+)", Color.green);
        EnterRoom();
    }
    ...
    public void OnPlayerEnteredRoom(Player newPlayer) {
        ...
        LocalChatMessage(newPlayer.GetUniqueNickname() + " joined the lobby (+)", Color.green);
    ...
    public void OnPlayerLeftRoom(Player otherPlayer) {
        ...
        LocalChatMessage(otherPlayer.GetUniqueNickname() + " left the lobby (-)", Color.red);
    ...
    private void RunCommand(string[] args) {
        ...
        case "debug": {
            ...
            if (debugEnabled) {
                LocalChatMessage("Debug and fun features have been disabled.", Color.red);
            } else {
                LocalChatMessage("Debug and fun features have been enabled!", Color.red);
            }
            PhotonNetwork.CurrentRoom.SetCustomProperties(new() {
        ...
        case "showrank": {
                LocalChatMessage("<color=#FF7700>Your current global rank is: " + PlayerPrefs.GetInt("PlayerRank", 1000) + "</color>", Color.red);
                return;
            }

        // These 2 commands are note'd for now so i dont accidentally end up making a build where people can use these....
        
        /*case "debugsetrank": {
                PlayerPrefs.SetInt("PlayerRank", Random.Range(1000, 5000));
                PlayerPrefs.Save();
                LocalChatMessage("<color=#FF7700>You have set your current global rank to: " + PlayerPrefs.GetInt("PlayerRank", 1000) + "</color>", Color.red);
                return;
            }

        case "debugresetrank": {
            PlayerPrefs.SetInt("PlayerRank", (1000));
            PlayerPrefs.Save();
            LocalChatMessage("<color=#FF7700>You have successfully reset your global rank to: " + PlayerPrefs.GetInt("PlayerRank", 1000) + "</color>", Color.red);
            return;
        */ }
    ...
    public void SetTime(TMP_InputField input) {
        ...
        if (seconds >= 180)
            LocalChatMessage("Time set to: " + timeField.text + " (Ranked is enabled).", Color.blue);
        else
            LocalChatMessage("Time set to: " + timeField.text + " (Ranked is disabled).", Color.blue);
